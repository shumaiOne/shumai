...
export function PdfControlBar({
  ...
  return (
    <div className="relative px-4 py-3 bg-card border-t border-gray-200 dark:border-gray-700 z-10 flex items-center justify-between gap-2 transition-colors duration-200">
      {/* Page Navigation */}
      <div className="flex items-center gap-1.5 bg-gray-200/50 dark:bg-white/10 rounded-md p-0.5">
        <button
          onClick={() => onPageChange(Math.max(1, currentPage - 1))}
          disabled={currentPage <= 1}
          className="p-1.5 rounded hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 transition-colors disabled:opacity-40"
          title={m.previous_page()}
        >
          <ChevronLeft size={16} />
        </button>
        <span className="px-2 text-xs font-mono font-medium text-gray-900 dark:text-gray-100 select-none">
