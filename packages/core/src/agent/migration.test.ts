import { describe, expect, it } from 'vitest'
import { prisma } from '@shumai/db'
import { setupTestDbHooks } from '@shumai/db/test'
import { migrateLegacyAgentAvatars } from './migration'
import { getPresetAvatarBuffer } from './presets'

describe('migrateLegacyAgentAvatars', () => {
  setupTestDbHooks()

  it('returns 0 when no legacy agents exist', async () => {
    const result = await migrateLegacyAgentAvatars(prisma)
    expect(result).toEqual({ migrated: 0, errors: 0 })
  })

  it('migrates legacy agent base64 avatars to S3', async () => {
    const presetBase64 =
      'data:image/webp;base64,' + getPresetAvatarBuffer('avatar-1')!.toString('base64')
    const customBase64 =
      'data:image/webp;base64,'

    const agentUser = await prisma.user.create({
      data: {
        name: 'Legacy Agent Preset',
        email: `legacy-preset-${Date.now()}@shumai.ai`,
        type: 'agent',
        image: presetBase64,
      },
    })

    const customAgentUser = await prisma.user.create({
      data: {
        name: 'Legacy Agent Custom',
        email: `legacy-custom-${Date.now()}@shumai.ai`,
        type: 'agent',
        image: customBase64,
      },
    })

    const humanUser = await prisma.user.create({
      data: {
        name: 'Human User',
        email: `human-${Date.now()}@shumai.ai`,
        type: 'human',
        image: customBase64,
      },
    })

    const modernAgentUser = await prisma.user.create({
      data: {
        name: 'Modern Agent',
        email: `modern-${Date.now()}@shumai.ai`,
        type: 'agent',
        image: 'files/existing-key.webp',
      },
    })

    const team = await prisma.team.create({
      data: { name: 'Test Team' },
    })

    await prisma.agent.create({
      data: {
        id: agentUser.id,
        teamId: team.id,
        type: 'chat',
        config: {
          provider: 'p',
          model: 'm',
        },
      },
    })

    await prisma.agent.create({
      data: {
        id: customAgentUser.id,
        teamId: team.id,
        type: 'chat',
        config: {
          provider: 'p',
          model: 'm',
        },
      },
    })

    const result = await migrateLegacyAgentAvatars(prisma)
    expect(result).toEqual({ migrated: 2, errors: 0 })

    const updatedAgent = await prisma.user.findUnique({
      where: { id: agentUser.id },
    })
    expect(updatedAgent?.image).toMatch(/^files\/[A-Z0-9]{26}\.webp$/)

    const updatedAgentRecord = await prisma.agent.findUnique({
      where: { id: agentUser.id },
    })
    expect((updatedAgentRecord?.config as unknown as PrismaJson.AgentConfig)?.avatarPreset).toBe(
      'avatar-1',
    )

    const updatedCustomAgent = await prisma.user.findUnique({
      where: { id: customAgentUser.id },
    })
    expect(updatedCustomAgent?.image).toMatch(/^files\/[A-Z0-9]{26}\.webp$/)

    const updatedCustomAgentRecord = await prisma.agent.findUnique({
      where: { id: customAgentUser.id },
    })
    expect(
      (updatedCustomAgentRecord?.config as unknown as PrismaJson.AgentConfig)?.avatarPreset,
    ).toBeUndefined()

    const updatedHuman = await prisma.user.findUnique({
      where: { id: humanUser.id },
    })
    expect(updatedHuman?.image).toBe(customBase64)

    const updatedModern = await prisma.user.findUnique({
      where: { id: modernAgentUser.id },
    })
    expect(updatedModern?.image).toBe('files/existing-key.webp')
  })
})
