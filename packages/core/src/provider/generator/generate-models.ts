...
} from './models-dev-reasoning-options.ts'
import {
  CLOUDFLARE_AI_GATEWAY_ANTHROPIC_BASE_URL,
  CLOUDFLARE_AI_GATEWAY_COMPAT_BASE_URL,
  CLOUDFLARE_AI_GATEWAY_OPENAI_BASE_URL,
  CLOUDFLARE_WORKERS_AI_BASE_URL,
} from '@earendil-works/pi-ai/api/cloudflare'
import type {
...
const GITHUB_COPILOT_THINKING_LEVEL_OVERRIDES: Record<
  string,
...
): OpenAICompletionsResolvedCompat {
...
function openAICompletionsCompatDelta(
  compat: OpenAICompletionsResolvedCompat,
): OpenAICompletionsCompat {
  const delta: OpenAICompletionsCompat = {}
  for (const [key, value] of Object.entries(compat)) {
    const defaultValue =
      OPENAI_COMPLETIONS_DEFAULT_COMPAT[key as keyof typeof OPENAI_COMPLETIONS_DEFAULT_COMPAT]
...
function applyOpenAIGrammarToolCompatMetadata(model: Model<Api>): void {
  if (
    !OPENAI_GRAMMAR_TOOL_APIS.has(model.api) ||
    !OPENAI_GRAMMAR_TOOL_PROVIDERS.has(model.provider)
  )
    return
...
function applyThinkingLevelMetadata(model: Model<any>): void {
  ...
  if (
    model.provider === 'xai' &&
    model.api === 'openai-responses' &&
    model.id === XAI_RESPONSES_MODEL_ID
  ) {
  ...
  try {
    ...
    if (data['google-vertex']?.models) {
      for (const [modelId, model] of Object.entries(data['google-vertex'].models)) {
        ...
        if (modelId === 'gemini-flash-latest') {
          source =
            (data['google-vertex'].models['gemini-3.5-flash'] as ModelsDevModel | undefined) ?? m
        }
        if (modelId === 'gemini-flash-lite-latest') {
          source =
            (data['google-vertex'].models['gemini-3.1-flash-lite'] as ModelsDevModel | undefined) ??
            m
        }
...
export async function generateModels(
  ...
  for (const candidate of allModels) {
    if (
      ...
      GITHUB_COPILOT_EXTENDED_CONTEXT_MODELS.has(candidate.id)
    ) {
    ...
    if (
      ...
      OPENAI_SHORT_CONTEXT_CAPPED_MODEL_IDS.has(candidate.id)
    ) {
