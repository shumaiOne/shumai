...
export const readSkillTool = defineTool<typeof readSkillSchema, { skillId: string }>({
  name: 'read_skill',
  ...
  execute: async (_toolCallId, params) => {
    try {
      ...
      if (needsDownload) {
        ...
        const { buffer: zipBuffer } = await s3Service.getObject(process.env.S3_BUCKET || 'shumai', asset.key)
