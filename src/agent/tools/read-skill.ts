...
export const createReadSkillTool = (sessionManager: DatabaseSessionManager) =>
  defineTool<typeof readSkillSchema, { skillId: string }>({
    name: 'read_skill',
    ...
    execute: async (_toolCallId, params) => {
      try {
        ...
        if (config?.environmentVariables && Array.isArray(config.environmentVariables)) {
          ...
          sessionManager.addSkillEnvs(envs)
        }
