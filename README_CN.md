...
> [!NOTE]
> **媒体处理与存储说明：** 在上传媒体文件时，Shumai 会始终完整保留并存储您的原始文件。对于所有媒体类型（图片与视频），Shumai 都会统一生成一张 480p 的 WebP 预览图，用作文件列表页预览。此外：
> * **图片文件：** Shumai 会额外生成一个与原图分辨率完全一致的 WebP 优化版本。
> * **视频文件：** 系统会根据您的转码设置生成至少一个预览版本mp4，同时还会生成一张雪碧图（Sprite）和一个轻量级预览视频（上限 1600 帧），以实现流畅的文件列表页面预览和文件查看页面时间轴快速预览。
